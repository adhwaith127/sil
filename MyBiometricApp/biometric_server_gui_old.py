#!/usr/bin/env python3
"""
Biometric Server Desktop Application with GUI
Maintains all original functionality while adding a user-friendly interface
"""
...
# ============================================================================
# ORIGINAL SERVER CODE (with minor modifications for GUI integration)
# ============================================================================
...
class EnhancedLogger:
    """Modified Enhanced Logger with GUI support"""
...
class BiometricServer:
    """Modified BiometricServer with GUI support"""
    ...
    async def handle_device(self, websocket, path=None):
        ...
        try:
            ...
            if websocket in self.connected_devices:
                ...
                if self.enhanced_logger:
                    self.enhanced_logger.log_system_event("DEVICE_UNREGISTERED", f"Device disconnected: {serial}")
...
class BiometricServerGUI:
    """Main GUI Application for Biometric Server"""
    
    def __init__(self):
        self.config_manager = config_manager  # Use the global config manager
        self.root = tk.Tk()
        self.root.title("Biometric Server Control Panel")
    ...
    def setup_gui(self):
        ...
        # Title
        title_label = ttk.Label(header_frame, text="Biometric Server Control Panel", 
        ...
        self.folder_button = ttk.Button(button_frame, text="Open Logs Folder", 
        ...
        # Connected devices
        self.devices_label = ttk.Label(stats_frame, text="Connected Devices: 0", 
                                      font=('Segoe UI', 9))
        self.devices_label.grid(row=0, column=0, padx=(0, 20))
        
        # Queue size
        self.queue_label = ttk.Label(stats_frame, text="Queue Size: 0", 
                                    font=('Segoe UI', 9))
        self.queue_label.grid(row=0, column=1, padx=(0, 20))
        
        # Uptime
        self.uptime_label = ttk.Label(stats_frame, text="Uptime: 00:00:00", 
                                     font=('Segoe UI', 9))
        self.uptime_label.grid(row=0, column=2)
        ...
        self.auto_scroll_check = ttk.Checkbutton(log_frame, 
                                                text="Auto-scroll", 
                                                variable=self.auto_scroll_var)
        self.auto_scroll_check.grid(row=1, column=0, sticky=tk.W, pady=(5, 0))
    ...
    def start_server(self):
        """Start the biometric server in a background thread"""
        if self.is_running:
            return
    ...
    def update_status(self):
        ...
        if self.is_running and self.server:
            try:
                ...
                device_count = len(self.server.connected_devices)
