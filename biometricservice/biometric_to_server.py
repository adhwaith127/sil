...
class SQLiteQueueManager:
    """
    SQLite-based queue manager that provides:
    1. Thread-safe operations using SQLite's built-in locking
    2. ACID transactions for data integrity
    3. Better performance for concurrent operations
    4. Human-readable data for debugging
    """
    
    def __init__(self, db_path):
        self.db_path = db_path
        self.init_database()
        # Using asyncio.Lock for better async compatibility
    ...
    def init_database(self):
        ...
        # Create indexes for faster queries (indexing explained later)
    ...
    async def save_to_queue(self, record_data):
        """Save failed record to queue - thread-safe using SQLite transactions"""
        async with self.lock:  # asyncio.Lock for async operations
            try:
                conn = sqlite3.connect(self.db_path)
                conn.execute('''
...
class BiometricServer:
    ...
    async def retry_queued_records(self):
        """Background task to retry queued records using SQLite"""
            ...
            try:
                ...
                for record in pending_records:
                    ...
                    if success:
                        ...
                        # Still a queue-worthy error - update retry count
