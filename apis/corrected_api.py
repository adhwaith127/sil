...
# Check if a date is working day - respect holiday list completely
def _is_working_day_for_employee(check_date, emp_holidays):
    """
    A day is a working day if it's NOT in the employee's holiday list.
    Holiday list should include all non-working days (including weekends if applicable).
    """
    return check_date not in emp_holidays
...
def _calculate_employee_work_hours(logs, shift_end=None):
    try:
        ...
        return {"error": "System error occurred."} first_in_time is None:
...
def _calculate_daily_work_hours_with_status(logs, employee_holidays, employee_leaves, date_str, employee_info):
    ...
    if not logs:
        ...
        # Priority order: Leave > Holiday > Absent
    ...
    # If logs exist, employee was present (even if it was a holiday/leave day)
    status = "Present"
...
def _calculate_effective_working_days(start_date, end_date, emp_holidays, emp_leaves, emp_working_days, checkin_dates, is_current_period=False):
    try:
        ...
        # For current periods, calculate up to yesterday
        ...
        # Count holidays and leaves that overlap in the period
        ...
        # Combine holidays and leaves, removing duplicates (same day can't be both)
        non_working_days = set(holidays_in_period + leaves_in_period)
        
        # Effective working days = Total days - Non-working days
...
# UPDATED: Calculate week/month boundaries with refined current vs past logic
def _get_date_boundaries(target_date: date) -> Dict[str, date]:
    try:
        ...
        # Calculate week boundaries (full 7-day week: Monday to Sunday)
        week_start = target_date - timedelta(days=target_date.weekday())
        week_end = week_start + timedelta(days=6)  # Sunday
        
        # Determine if periods are current or past
        ...
        # REFINED LOGIC: Different handling for week vs month
        
        # For MONTHLY: If current month, always go up to yesterday regardless of selected date
        if is_current_month:
            month_end = yesterday
            
        # For WEEKLY: If current week, go up to yesterday. If past week, use full week.
        if is_current_week:
            week_end = yesterday
        # If past week, week_end remains as calculated (full week)
...
# Main API endpoint for fetching checkin data with refined logic
@frappe.whitelist()
