...
@frappe.whitelist(allow_guest=True)
def get_employee_details(session_user, from_date=None, to_date=None, specific_date=None, reportee_name=None):
    try:
        # Validation logic
        ...
        # NEW MODE: Specific reportee with date range
        if reportee_name and from_date and to_date:
            try:
                ...
                # Get manager_id from session_user
                manager_id = frappe.db.get_value("Employee", {"user_id": session_user}, "name")
                if not manager_id:
                    frappe.throw("User not linked to active employee record.")

                # Validate reportee exists and is a subordinate
                hierarchy_map = _get_hierarchy_map()
                subordinate_ids = _get_all_subordinates(manager_id, hierarchy_map)
                
                if reportee_name not in subordinate_ids:
                    frappe.throw(f"Employee {reportee_name} is not a subordinate or does not exist.")

                # Get processed data for the entire date range (all employees)
                processed_data, employee_holidays, employee_leaves = _get_processed_checkin_data(start_date, end_date)
                
                # Filter data to include only the specific reportee
                reportee_data = [record for record in processed_data if record['employee'] == reportee_name]
                
                if not reportee_data:
                    # Even if no checkin data, we need to create records for absent/holiday/leave days
                    reportee_employee = frappe.get_value("Employee", reportee_name, 
                        ["name", "department", "reports_to", "image", "employee_name", "custom_team"], as_dict=True)
                    
                    if reportee_employee:
                        employee_info = _build_employee_info(reportee_employee)
                        reportee_holidays = employee_holidays.get(reportee_name, [])
                        reportee_leaves = employee_leaves.get(reportee_name, {}) if employee_leaves else {}
                        ...
                        while current_date <= end_date:
                            ...
                            daily_summary = _calculate_daily_work_hours_with_status(
                                None, reportee_holidays, reportee_leaves, date_str, employee_info
                            )
                            if daily_summary:
                                reportee_data.append(daily_summary)
                            current_date += timedelta(days=1)

                if not reportee_data:
                    return {"message": f"No data found for employee {reportee_name} between {from_date} and {to_date}."}
                
                # Create summary for the specific reportee
                is_current_period = end_date >= getdate(today())
                reportee_summary = _create_summary_with_effective_working_days(
                    reportee_data, start_date, end_date, 
                    {reportee_name: employee_holidays.get(reportee_name, [])}, 
                    {reportee_name: employee_leaves.get(reportee_name, {})} if employee_leaves else {}, 
                    is_current_period
                )
                
                # Add employee info to the summary
                if reportee_summary:
                    reportee_employee = frappe.get_value("Employee", reportee_name, 
                        ["name", "department", "reports_to", "image", "employee_name", "custom_team"], as_dict=True)
                    
                    for summary in reportee_summary:
                        summary.update({
                            "emp_display_name": reportee_employee.get('employee_name'),
                            "department": reportee_employee.get('department'),
                            "reports_to": reportee_employee.get('reports_to'),
                            "image": reportee_employee.get('image'),
                            "custom_team": reportee_employee.get('custom_team')
                        })
                
                return {
                    "reportee_name": reportee_name,
                    "date_range": f"{from_date} to {to_date}",
                    "summary": reportee_summary[0] if reportee_summary else {},
                    "daily_records": reportee_data
                }
                
            except Exception as e:
                frappe.log_error("Error in reportee date range processing", str(e))
                return {"error": "Failed to process reportee date range request"}

        # EXISTING MODE: Date range for session_user's hierarchy  
            ...
            try:
                ...
                # Get hierarchy - manager + subordinates
                hierarchy_map = _get_hierarchy_map()
                subordinate_ids = _get_all_subordinates(manager_id, hierarchy_map)
                allowed_employees = set(subordinate_ids + [manager_id])

                # Get processed data for the entire date range
                processed_data, employee_holidays, employee_leaves = _get_processed_checkin_data(start_date, end_date)
                
                # Filter data to include only hierarchy employees
                hierarchy_data = [record for record in processed_data if record['employee'] in allowed_employees]
                
                # If no processed data for hierarchy employees, we still need to create records
                if not hierarchy_data:
                    ...
                    for emp in all_employees:
                        employee_info = _build_employee_info(emp)
                        emp_holidays = employee_holidays.get(emp.name, [])
                        emp_leaves = employee_leaves.get(emp.name, {}) if employee_leaves else {}
                        ...
                        while current_date <= end_date:
                            date_str = current_date.strftime('%Y-%m-%d')
                            daily_summary = _calculate_daily_work_hours_with_status(
                                None, emp_holidays, emp_leaves, date_str, employee_info
                            )
                            if daily_summary:
                                hierarchy_data.append(daily_summary)
                            current_date += timedelta(days=1)
                
                if not hierarchy_data:
                    return {"message": f"No data found for your team between {from_date} and {to_date}."}
                
                is_current_period = end_date >= getdate(today())
                
                # Filter employee_holidays and employee_leaves to only include hierarchy employees
                filtered_holidays = {emp: holidays for emp, holidays in employee_holidays.items() if emp in allowed_employees}
                filtered_leaves = {emp: leaves for emp, leaves in employee_leaves.items() if emp in allowed_employees} if employee_leaves else {}
                    
                return _create_summary_with_effective_working_days(
                    hierarchy_data, start_date, end_date, 
                    filtered_holidays, filtered_leaves, is_current_period
                )
        ...
        # EXISTING MODE: Specific date with hierarchy
        elif specific_date:
            try:
