...
class EnhancedLogger:
    ...
    def setup_category_loggers(self):
        ...
        self.queue_logger.handlers.clear()
    ...
    def shutdown(self):
        ...
        try:
            # Close all handlers for each logger
            for logger in [self.main_logger, self.success_logger, self.error_logger, 
                          self.queue_logger, self.system_logger]:
...
class SQLiteQueueManager:
    """
    Enhanced SQLite-based queue manager with:
    1. Proper WAL mode for better concurrency
    2. Connection pooling and proper locking
    3. Better error handling and duplicate prevention
    ...
    def __init__(self, db_path):
        ...
        self.init_database()
        # Using threading.RLock for true thread safety across sync/async
    ...
    def get_connection(self):
        ...
        if self._closed:
            raise sqlite3.Error("Database manager has been closed")
    ...
    def init_database(self):
        ...
        try:
            conn.execute('BEGIN IMMEDIATE;')  # Exclusive lock for schema changes
            ...
            conn.execute('''
                CREATE TABLE IF NOT EXISTS pending_checkins (
                    ...
                    UNIQUE(employee_id, timestamp, device_id) ON CONFLICT IGNORE
                )
            ''')
            ...
            conn.execute('CREATE INDEX IF NOT EXISTS idx_queued_at ON pending_checkins(queued_at)')
            
            # Add a view for easier querying from external tools
            conn.execute('''
                ...
                SELECT 
                    ...
                    last_error,
                    CASE 
                        WHEN retry_count >= 10 THEN 'MAX_RETRIES_REACHED'
                        WHEN datetime('now') > datetime(queued_at, '+24 hours') THEN 'EXPIRED'
                        ELSE 'ACTIVE'
                    END as queue_status
                ...
                SELECT 
                    ...
                    timestamp,
                    queued_at,
                    total_attempts as retry_count,
                    final_error as last_error,
                    failure_reason as queue_status
                FROM failed_checkins
    ...
    async def save_to_queue(self, record_data):
        """Save failed record to queue with duplicate prevention"""
        with self.lock:
            if self._closed:
                return False
            ...
            try:
                ...
                # Check if this exact record already exists (duplicate prevention)
                cursor = conn.execute('''
                ...
                if cursor.fetchone():
                    enhanced_logger.log_queue_operation(
                        ...
                        record_data['enrollid'], 
                        record_data['name'],
                        f"Record already in queue - skipping duplicate"
                    )
                    conn.close()
    ...
    def get_pending_records(self):
        """Get all pending records - returns list of dictionaries for easy reading"""
        if self._closed:
            return []
    ...
    async def update_retry_count_and_error(self, record_id, error_message=None):
        """Update retry count and error for a record - NO re-queuing happens here"""
        with self.lock:
            if self._closed:
                return False
            ...
            try:
                ...
                # Get current record info for logging
                cursor = conn.execute('SELECT employee_id, employee_name, retry_count FROM pending_checkins WHERE id = ?', (record_id,))
                record = cursor.fetchone()
                
                if record:
                    new_retry_count = record[2] + 1
                    ...
                    enhanced_logger.log_queue_operation(
                        ...
                        f"Retry count updated to {new_retry_count}. Last error: {error_message}"
                    )
                ...
                enhanced_logger.log_error(
                    "SYSTEM", "SYSTEM", "SYSTEM",
                    f"Failed to update retry count: {str(e)}",
                    "DATABASE_ERROR"
                )
                return False
    ...
    def close(self):
        ...
        with self.lock:
            self._closed = True
            enhanced_logger.log_system_event("DATABASE_CLOSED", "SQLite queue manager closed")
...
class BiometricServer:
    ...
    async def store_attendance(self, records, device_id):
        """Process attendance records with SQLite queue integration"""
        if not records:
            enhanced_logger.log_error(
    ...
    async def retry_queued_records(self):
        """Background task to retry queued records using SQLite with NO re-queuing"""
        ...
        while not self.shutdown_event.is_set():
            ...
            try:
                ...
                for record in pending_records:
                    ...
                    # Try to resend - CRITICAL: This is where retry happens
                    ...
                    if success:
                        # Success - remove from queue (NO re-adding)
                        ...
                        # Still a queue-worthy error - update retry count ONLY (NO re-queuing)
        ...
        enhanced_logger.log_system_event("RETRY_TASK_STOPPED", "Queue retry task stopped gracefully")
    ...
    async def handle_device(self, websocket, path=None):
        ...
        try:
            ...
            if not self.shutdown_event.is_set():
                enhanced_logger.log_error(
                    "SYSTEM", "SYSTEM", self.connected_devices.get(websocket, "unknown"),
                    f"Device handler error: {e}", "CONNECTION_ERROR"
                )
        finally:
    ...
    async def graceful_shutdown(self, signum=None):
        ...
        # Set shutdown event to stop retry task
        ...
        if self.connected_devices:
            enhanced_logger.log_system_event(
                "CLOSING_CONNECTIONS", 
                f"Closing {len(self.connected_devices)} active device connections"
            )
        ...
        if self.retry_task and not self.retry_task.done():
            enhanced_logger.log_system_event("WAITING_RETRY_TASK", "Waiting for retry task to complete")
            try:
                await asyncio.wait_for(self.retry_task, timeout=10.0)
            except asyncio.TimeoutError:
                enhanced_logger.log_system_event("RETRY_TASK_TIMEOUT", "Retry task did not complete in time")
                self.retry_task.cancel()
        
        # Close server
        if self.server:
            enhanced_logger.log_system_event("CLOSING_SERVER", "Closing WebSocket server")
            self.server.close()
        ...
        enhanced_logger.log_system_event("SHUTDOWN_COMPLETE", "Server shutdown completed gracefully")
    ...
    async def start_server(self):
        ...
        def signal_handler(signum, frame):
            enhanced_logger.log_system_event("SIGNAL_RECEIVED", f"Received signal {signum}")
            asyncio.create_task(self.graceful_shutdown(signum))
        ...
        try:
            ...
            # Windows or other systems might not support these signals
            pass
...
def log_attendance_to_csv(enroll_id, name, device_id, status, timestamp_str):
    """Log attendance with enhanced status tracking"""
    try:
...
def cleanup_on_exit():
    ...
    try:
        enhanced_logger.log_system_event("EXIT_CLEANUP", "Performing cleanup on exit")
...
def main():
    ...
    try:
        ...
        enhanced_logger.log_system_event("APPLICATION_INTERRUPTED", "Server interrupted by user (Ctrl+C)")
    except Exception as e:
        enhanced_logger.log_error(
            "SYSTEM", "SYSTEM", "SYSTEM",
        ...
        # Cleanup is handled by atexit and graceful_shutdown
