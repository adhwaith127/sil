    ...
    def send_to_erp(self, punchingcode, name, timestamp_str, device_id):
        ...
        try:
            res = self.session.post(
                f"{ERP_URL}/{ERP_API}", data=payload, timeout=5
            )
            ...
            logger.info(
                f"ERP Checkin log added for {name} at {payload['time']}"
            )
            return True
        ...
        try:
            ...
            if "cmd" in data:
                ...
                if cmd == "reg":
                    ...
                    return await self.store_attendance(
                        data.get("record", []), device_id
                    )
...
def log_attendance_to_csv(enroll_id, name, device_id, status, timestamp_str):
    ...
    with open(filepath, mode="a", newline="") as f:
        ...
        if not file_exists:
            writer.writerow(
                ["Timestamp", "Enroll ID", "Name", "Device ID", "ERP Status"]
            )
        writer.writerow([
