...
# Get employee shift working days info - NEW ADDITION from api_test.py
...
# Get employee checkin data with shift info - UPDATED to include working_days
...
# Check if a date is working day for specific employee - NEW ADDITION
...
def _calculate_daily_work_hours_with_status(logs, employee_holidays, employee_leaves, date_str, employee_info):
    ...
    if not logs:
        ...
        if date_str in [str(d) for d in employee_holidays]:
            ...
            status = "On Leave"
        elif check_date.weekday() == 6:  # Sunday - NEW ADDITION
            status = "Off Day"
        else:
            status = "Absent"
    ...
    if date_str in employee_leaves:
        ...
        status = "Holiday"
    elif work_summary.get('entry_time') or work_summary.get('exit_time'):
        status = "Present"
    else:
        status = "Absent"
...
# UPDATED: Calculate effective working days - ensures all worked days are counted as valid working days
def _calculate_effective_working_days(start_date, end_date, emp_holidays, emp_leaves, emp_working_days, checkin_dates, is_current_period=False):
    try:
        ...
        # For current periods, use "up to yesterday" logic from checkin_dummy.py
        ...
        while current_date <= actual_end_date:
            # If employee checked in on this date, it's definitely a valid working day (including weekends/holidays)
            if str(current_date) in checkin_dates:
                effective_working_days += 1
            else:
                # Employee didn't check in - check if it should have been a working day
                if _is_working_day_for_employee(current_date, emp_holidays):
                    # It's a regular working day, check if not on leave
                    if current_date not in emp_leave_dates:
                        effective_working_days += 1
                # Weekends and holidays without check-ins are not counted as working days
...
def _get_processed_checkin_data(from_date: date, to_date: date) -> List[Dict[str, Any]]:
    try:
        ...
        employee_shift_info = _get_employee_shift_info()  # NEW ADDITION
...
# UPDATED: Create period summary with effective working days calculation (integrating checkin_dummy.py logic)
def _create_summary_with_effective_working_days(daily_records: List[Dict[str, Any]], period_type: str, start_date: date, end_date: date, employee_holidays: dict, employee_leaves: dict, employee_shift_info: dict, is_current_period: bool = False) -> List[Dict[str, Any]]:
    try:
        ...
        for emp_name, stats in employee_stats.items():
            ...
            # Calculate effective working days using improved logic from checkin_dummy.py
...
# UPDATED: Calculate week/month boundaries with current vs past logic (from api_test.py and checkin_dummy.py)
def _get_date_boundaries(target_date: date) -> Dict[str, date]:
    try:
        ...
        # Calculate week boundaries (Monday to Saturday for 6-day week)
        ...
        # For current periods, boundaries end at yesterday (from checkin_dummy.py logic)
...
# UPDATED: Process and categorize data into daily/weekly/monthly periods with improved logic
def _process_data_by_periods(registry: Dict, all_data: List[Dict], boundaries: Dict, target_date: date, employee_holidays: dict, employee_leaves: dict, employee_shift_info: dict):
    try:
        if not all_data:
            all_data = []
        ...
        # Process daily data with status
        if daily_data:
            _add_to_registry(registry, daily_data, 'daily_data')

        # Process weekly and monthly summaries with new effective working days logic
        if weekly_data:
        ...
        # Ensure all employees have daily, weekly, and monthly records with fallback logic
...
# Main API endpoint for fetching checkin data with integrated improvements
@frappe.whitelist()
def fetch_checkins(from_date: str = None, to_date: str = None, specific_date: str = None) -> Dict[str, Any]:
    try:
        ...
        # Handle date range request with improved logic
        if from_date and to_date:
            try:
                ...
                # Calculate period type and determine if current period
        ...
        # Handle specific date request with all improvements integrated
        elif specific_date:
            try:
                ...
                # Process data by periods with effective working days logic
