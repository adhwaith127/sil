...
def _get_employee_shift_info():
    try:
        query = """
            SELECT em.name as employee, COALESCE(st.custom_working_days, 5) as working_days
            FROM `tabEmployee` em
...
# Check if a date is working day for specific employee based on their shift and holiday list
def _is_working_day_for_employee(check_date, employee_holidays):
    weekday=check_date.weekday()

    if weekday==6:
        return False
    
    if check_date in employee_holidays:
        return False
...
def _filter_regular_work_checkins(raw_checkin_data, employee_holidays):
    try:
        ...
        frappe.log_error("Error in filtering checkin data",str(e))
        return [],[]
...
def _calculate_employee_work_hours(logs, shift_end=None):
    try:
        ...
        # Define auto-close time (11:59 PM of the log date)
        auto_close_time = datetime.combine(log_date, datetime.strptime("23:59", "%H:%M").time())
        
        # Use shift end if available, otherwise use 23:59
        if shift_end:
            effective_close_time = min(shift_end, auto_close_time)
        else:
            effective_close_time = auto_close_time
        ...
        if last_in_time:
            # Only auto-close if current time has passed the effective close time
            # AND it's not today, OR if it's today and we're past close time
            should_auto_close = (
                (log_date < current_date) or  # Past date - always auto close
                (log_date == current_date and current_time >= effective_close_time)  # Today but past close time
            )
            ...
            if should_auto_close:
                ...
                checkin_pairs.append({
                    ...
                    "duration": round(session_duration, 2),
                    "auto_closed": True  # Mark as auto-closed
                })
                # Set exit time to the auto-close time
                last_out_time = effective_close_time
            else:
                # Leave session open - show ongoing session
                checkin_pairs.append({
                    "in_time": last_in_time.strftime("%H:%M"),
                    "out_time": "Ongoing",
                    "duration": 0.0,
                ...
                # Don't set exit time for ongoing sessions
                pass
...
# Enhanced daily work hours calculation with status and leave handling (updated)
...
def _calculate_dynamic_working_days(start_date, end_date, employee_name, employee_holidays, employee_leaves, employee_shift_info, all_checkin_dates, is_current_period=False):
    try:
        ...
        emp_leave_dates=[getdate(d_str) for d_str in employee_leaves]
        ...
        while current_date<=actual_end_date:
            ...
            is_working_day = _is_working_day_for_employee(current_date, employee_holidays)
...
def _create_summary_with_dynamic_working_days(daily_records, period_type, start_date, end_date, employee_holidays, employee_leaves, employee_shift_info, is_current_period=False):
    try:
        ...
        for emp_name, stats in employee_stats.items():
            ...
            base_working_days = employee_shift_info.get(emp_name, 5)
